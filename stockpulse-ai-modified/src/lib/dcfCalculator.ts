...
export function calculateDCF(data: DCFInputData): DCFResult {
  ...
  steps.push({
    ...
    formula: '基础数据输入',
    calculation: `当前FCF: ${data.currentFCF}亿元, 现金: ${data.cashAndEquivalents}亿元, 负债: ${data.totalDebt}亿元`,
    result: '数据确认完成',
    details: [
      `当前年度自由现金流 (FCF): ${data.currentFCF.toFixed(2)} 亿元`,
      `现金及现金等价物: ${data.cashAndEquivalents.toFixed(2)} 亿元`,
      `总负债: ${data.totalDebt.toFixed(2)} 亿元`,
  ...
  steps.push({
    ...
    calculation: `预测期: ${data.projectionYears}年, 总现值: ${totalPV.toFixed(2)}亿元`,
    result: totalPV,
    details: projections.map(p => 
      `${p.yearLabel}: FCF=${p.fcf.toFixed(2)}亿元, 增长率=${p.growthRate}%, 折现因子=${p.discountFactor.toFixed(4)}, 现值=${p.presentValue.toFixed(2)}亿元`
    )
  });
  ...
  steps.push({
    ...
    details: [
      `最后一年FCF (FCFₙ): ${lastFCF.toFixed(2)} 亿元`,
      ...
      `终值 (TV): ${terminalValue.toFixed(2)} 亿元`,
      `终值折现因子: ${terminalDiscountFactor.toFixed(4)}`,
      `终值现值: ${terminalValuePV.toFixed(2)} 亿元`,
    ]
  });
  ...
  steps.push({
    ...
    details: [
      `预测期现金流现值总和: ${totalPV.toFixed(2)} 亿元`,
      `终值现值: ${terminalValuePV.toFixed(2)} 亿元`,
      `企业价值 (EV): ${enterpriseValue.toFixed(2)} 亿元`,
    ]
  });
  ...
  steps.push({
    ...
    details: [
      `企业价值 (EV): ${enterpriseValue.toFixed(2)} 亿元`,
      `加: 现金及现金等价物: ${data.cashAndEquivalents.toFixed(2)} 亿元`,
      `减: 总负债: ${data.totalDebt.toFixed(2)} 亿元`,
      `净债务: ${(data.totalDebt - data.cashAndEquivalents).toFixed(2)} 亿元`,
      `股权价值: ${equityValue.toFixed(2)} 亿元`,
    ]
  });
